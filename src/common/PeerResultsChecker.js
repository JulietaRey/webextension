class PeerResultsChecker {
  ...
  triggerSearch() {
    ...
    for (let i = 0; i < this.totalPeers; i++) {
      ...
      setTimeout(()=>{
        this.peerResponses[i] = {finished: true, found: (randomDelay % 2 === 0) };
  ...
  getCompletedCount() {
    return this.peerResponses.reduce((tot, current)=> current.finished ?
      tot + 1 : tot
      , 0);
  }
  getFoundCount() {
    return this.peerResponses.reduce((tot, current)=> current.found ?
      tot + 1 : tot
      , 0);
  }
}